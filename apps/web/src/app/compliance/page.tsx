...
export default function CompliancePage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch(`${API_BASE_URL}/api/v1/platform/page/compliance`)
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        {loading ? (
          ...
          <div className="text-center py-20">
            <h1 className="text-2xl font-bold text-foreground mb-3">Compliance</h1>
            <p className="text-muted">A politica de compliance sera publicada em breve.</p>
          </div>
        )}
      </main>
    </div>
