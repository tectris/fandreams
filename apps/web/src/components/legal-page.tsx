...
export function LegalPage({ pageKey, fallbackTitle, fallbackMessage }: LegalPageProps) {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch(`${API_BASE_URL}/api/v1/platform/page/${pageKey}`)
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        {loading ? (
          ...
          <div className="text-center py-20">
            <h1 className="text-2xl font-bold text-foreground mb-3">{fallbackTitle}</h1>
            <p className="text-muted">{fallbackMessage || 'Este conteudo sera publicado em breve.'}</p>
          </div>
        )}
      </main>
    </div>
